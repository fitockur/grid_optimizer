...
bool Nearest3DInterpolator::spatial_convex_inclusion(const index & ijk_c, const index & ijk_vc) {
	...
	for (const auto &step : steps) {
		...
		for (size_t _ = 0; _ < 3; _++) {
			...
			if (this->plane_position(op, ap, bp, point) >= 0)
				return false;
		}
	}
	return true;
...
double Nearest3DInterpolator::plane_position(const alglib::real_1d_array & o, 
	...
	return ((a[2] - o[2]) * (b[1] - o[1]) - (a[1] - o[1]) * (b[2] - o[2])) * (point[0] - o[0]) +
		((a[0] - o[0]) * (b[2] - o[2]) - (a[2] - o[2]) * (b[0] - o[0])) * (point[1] - o[1]) +
		((a[1] - o[1]) * (b[0] - o[0]) - (a[0] - o[0]) * (b[1] - o[1])) * (point[2] - o[2]);
}
